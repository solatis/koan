...
1. Add the `(provider, model)` entry to `MODEL_CAPABILITIES` in `koan/agents/model_catalog.py` with `(thinking_modes, fallback_display_name)`.
2. If the model introduces a new family (e.g., `claude-fable`): add the family key to `_RECOGNIZED_FAMILIES` in `koan/agents/recognition.py` (a `frozenset[str]` of recognized family keys, membership-tested for the `recognized` flag), and add the family name to the `_RE_CLAUDE_NEW` regex alternation (for Claude-family models). Without both, `parse_model_id` returns `recognized=False` for the new model id.
