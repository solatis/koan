...
Bedrock's keyless-with-region construction path was later removed: a Bedrock connection now requires a stored long-lived API key, and `build_model` raises `AgentError(code="missing_credentials")` for a bedrock spec with no `api_key`, so the AWS credential chain is no longer a construction path.
