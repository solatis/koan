...
PHASE_ROLE_CONTEXT = (
    ...
    "examples, and an 8-item self-validation checklist appear in step 2\n"
...
def _tools_this_step_block(current_step: int) -> list[str]:
    ...
    if current_step == 2:
        return [
            "<tools_this_step>",
            "Writing discipline, two contrastive examples, and an 8-item",
...
def _step_2_memorize(ctx: PhaseContext) -> StepGuidance:
    instructions = _header(2) + [
        ...
        "",
        "**3. Attribute decisions, not claims.** For every decision in",
        "the entry, name WHO decided it: the user (\"user decided\",",
        "\"user directed\", \"user stated\") or the LLM (\"LLM inferred\",",
        "\"agent proposed\", \"plan-review concluded\"). User-decided facts",
        "carry more weight than LLM inferences; the distinction lets a",
        "future reader judge how strongly to trust the entry and detect",
        "issues (an LLM-inferred decision is more suspect than an",
        "explicit user direction). This rule applies to decisions only --",
        "do NOT attribute every individual claim to a conversation",
        "source. Background facts, lessons, and procedures should read",
        "as standalone knowledge, not as a record of who said what when.",
        "",
        ...
        "### B. Self-critique",
        "",
        "For each draft produced in substep A, run the 8-item draft-",
        ...
        "      3. Decisions attributed:              PASS / FAIL",
        "      4. Event-style, past tense:           PASS / FAIL",
        "      5. Concrete naming:                   PASS / FAIL",
        "      6. Project-stable identifiers only:   PASS / FAIL",
        "      7. Contains knowledge, not pointer:   PASS / FAIL",
        "      8. Passes code-comment test:          PASS / FAIL",
        "",
        "    Draft 2 ({title}):",
        "      ...",
        "",
        ...
        "Loop until all 8 items PASS for all drafts in the batch.",
        "",
        ...
        "",
        "For each draft, verify all 8 items. Any FAIL means the draft",
        ...
        "\"at the moment\" FAIL.",
        "",
        "**3. Decisions are attributed.**",
        "Every decision in the body names WHO decided it: the user",
        "(\"user decided\", \"user directed\", \"user stated\") or the LLM",
        "(\"LLM inferred\", \"agent proposed\", \"plan-review concluded\").",
        "Anonymous decisions (\"it was decided that...\" without a",
        "subject) FAIL. Non-decision content (background facts,",
        "lessons, procedures) does NOT need per-claim attribution and",
        "should read as standalone knowledge.",
        "",
        ...
        "",
        "**6. Project-stable identifiers only.**",
        ...
        "\"+72 LOC\").",
        "",
        "**7. Contains the knowledge, not a pointer.**",
        ...
        "other file.",
        "",
        "**8. Passes the code-comment test.**",
