...
def _discard_nonexecuted_artifacts(app_state: "AppState", run_dir: str) -> None:
    """Delete non-executed, non-frozen artifacts on milestone re-entry.

    Fires only when re-entering the "milestone" phase with an existing
    milestones.md (structural re-decomposition per brief Decision 16). Deletes
    draft plan artifacts and milestones.md plus their .review.md sidecars,
    preserving: brief.md, core-flows.md, tech-plan.md, every frozen/executed
    plan, and the sidecars of kept artifacts.

    Calls _push_artifact_diff after deletion so the projection emits
    artifact_removed events for each deleted file, keeping the sidebar consistent.

    This is destructive by design -- the discard hook is the mechanical trigger
    for re-decomposition. The user chose auto-trigger over confirmation (brief D16).
    """
    from ..driver import _push_artifact_diff

    root = Path(run_dir)
    frozen = _frozen_artifact_names(app_state)
    executed = _executed_artifact_names(app_state)
    # Always-preserved context files -- these are not regenerated by this phase.
    always_keep: frozenset[str] = frozenset({"brief.md", "core-flows.md", "tech-plan.md"})
    keep: frozenset[str] = frozen | executed | always_keep
    # Keep the .review.md sidecars of any kept artifact.
    kept_sidecars: frozenset[str] = frozenset(
        f"{name[:-3]}.review.md" for name in keep if name.endswith(".md")
    )
    keep_all = keep | kept_sidecars

    for artifact_path in list(root.glob("*.md")):
        name = artifact_path.name
        if name not in keep_all:
            artifact_path.unlink(missing_ok=True)

    # Push artifact diff to emit artifact_removed events for each deleted file.
    _push_artifact_diff(app_state)


async def apply_set_phase(deps: ToolDeps, phase: str, plan_file: str | None = None) -> str:
    ...
    When phase=="execute" and plan_file is provided, executes the mechanical
    handoff: validates the plan_file via the artifact registry (no side effects
    ...
    and their sidecars, preserving brief.md/core-flows.md/tech-plan.md and every
    ...
    # Milestone re-entry discard hook: fires ONLY when entering "milestone" and
    # milestones.md already exists (re-decomposition). Deletes non-executed draft
    # artifacts so milestone_spec always CREATEs fresh (brief 9.3, plan step 7).
    # Guard: skip on tech-plan re-entry (user decision D7 -- tech-plan.md edits in place).
    if phase == "milestone":
        run_dir_check = _resolve_run_dir_core(agent, app_state)
        if run_dir_check and (Path(run_dir_check) / "milestones.md").exists():
            _discard_nonexecuted_artifacts(app_state, run_dir_check)

    # Execute handoff: validate first (before any side effect), then freeze,
    # transition, spawn, complete. Bare set_phase("execute") without plan_file
    ...
    if phase == "execute":
        ...
        executed_names = _executed_artifact_names(app_state)

        err = artifact_registry.validate_execute_target(
            plan_file,
        ...
        # (b) Run the normal phase transition (validate, update driver state,
        # emit phase_started / yield_cleared / agent_step_advanced, rebuild ctx).
        # We fall through to the shared transition code below by NOT returning here.
        # The execute_entry is already emitted; the shared path commits the rest.
    ...
    # Execute handoff (continued): spawn the executor and push execute_completion.
    # This runs only after the normal phase-transition bookkeeping above so the
    # orchestrator is already in the "execute" phase when the executor runs.
    if phase == "execute" and plan_file:
        # (d) Build the artifact list: plan_file first, then standard context files.
        context_files = ("brief.md", "tech-plan.md", "core-flows.md", "milestones.md")
        artifacts = [plan_file] + [
            f for f in context_files
            if (Path(run_dir) / f).is_file() and f != plan_file
        ]

        # (e) Spawn the executor (blocking); returns (deviation_report, exit_code).
        deviation_report, exit_code = await _spawn_executor(app_state, run_dir, artifacts)

        # (f) Record completion outcome based solely on exit code (M4 contract).
        # The richer conformance determination (M5) may set non_conforming on a
        # clean-exit run; in M4 we use exit code as the sole discriminant.
        outcome = "clean" if exit_code == 0 else "non_conforming"
        app_state.projection_store.push_event(
            "execute_completion",
            {"plan_file": plan_file, "outcome": outcome},
        )

        # Return the deviation report as the tool result so the orchestrator can
        # read and act on it inline (the execute phase's inline conformance review reads this).
...
def _frozen_artifact_names(app_state: "AppState") -> frozenset[str]:
    """Return the set of artifact filenames currently frozen in the projection.

    Reads ArtifactInfo.frozen -- a derived field folded from execute_entry events.
    Returns an empty frozenset when there is no active run or no artifacts yet.
    """
    run = app_state.projection_store.projection.run
    if run is None:
        return frozenset()
    return frozenset(name for name, info in run.artifacts.items() if info.frozen)


def _executed_artifact_names(app_state: "AppState") -> frozenset[str]:
    """Return the set of artifact filenames already executed in the projection.

    Reads ArtifactInfo.executed -- a derived field folded from execute_completion
    events. Returns an empty frozenset when there is no active run.
    Used by validate_execute_target to reject repeat execution of the same plan.
    """
    run = app_state.projection_store.projection.run
    if run is None:
        return frozenset()
    return frozenset(name for name, info in run.artifacts.items() if info.executed)
...
async def _spawn_reviewer(
    ...
    Args:
        ...
        predecessor_chain: Ordered list of predecessor artifact filenames for
            remediation-context reviews (empty for first-pass reviews).
    """
    import time
    ...
    task = {
        ...
        "reviewer_predecessor_chain": predecessor_chain,
    }
...
async def _spawn_executor(
    ...
    The executor receives only the named artifacts; all context the executor
    needs must be in those files (brief Decision 5.2). instructions is always
    empty -- the plan is the sole instruction channel.
...
async def artifact_write_core(deps: ToolDeps, filename: str, content: str) -> str:
    ...
    # Reviewer configured: compute the predecessor chain for remediation-context
    # reviews (empty for first-pass writes).
    # predecessor_chain takes a list of all existing artifact names.
    chain = artifact_registry.predecessor_chain(filename, sorted(existing_names))

    # Spawn the reviewer (blocking) -- returns its freeform findings text.
    findings = await _spawn_reviewer(
        app_state, run_dir, filename, reviewer_prompt_tag, chain
    )

    # Persist findings to the koan-owned sidecar. The sidecar is written
    # directly to the filesystem (not via artifact_write_core) to bypass
    # the write-once validation -- sidecars are koan-owned, not orchestrator-named.
    sidecar_name = artifact_registry.sidecar_name_for(filename)
    sidecar_path = run_root / sidecar_name
    heading = "## Plan review (pre-exec)\n\n"
    sidecar_content = heading + findings
    sidecar_tmp = sidecar_path.with_suffix(".tmp")
    sidecar_tmp.write_text(sidecar_content, encoding="utf-8")
    os.rename(sidecar_tmp, sidecar_path)

    _push_artifact_diff(app_state)

    # Return the findings to the orchestrator so it can read and incorporate them.
    return findings
...
async def artifact_edit_core(
    ...
    # Sidecar filenames (e.g. plan.review.md) contain a dot before .md and are
    # deliberately rejected by _validate_artifact_filename (which requires
    # [a-z0-9][a-z0-9_-]*.md). We must resolve their path separately, using
    # _resolve_run_dir_core directly, and skip the normal _resolve_artifact_path
    # guard. The sidecar is always inside run_dir by construction (koan creates
    # it), so the containment property holds without the regex check.
    from ..tools import artifact_registry as _reg_mod
    if _reg_mod.is_review_sidecar(filename):
        run_dir_str = _resolve_run_dir_core(deps.agent, deps.app_state)
        if not run_dir_str:
            raise ValueError("no_run_dir: No run directory available")
        target = (Path(run_dir_str) / filename).resolve()
    else:
...
# request_executor_core and its koan_request_executor wrapper deleted in M4:
# execution now rides on koan_set_phase("execute", plan_file=...) which freezes
# the named plan, spawns the executor via _spawn_executor, and returns the
# deviation report. The standalone tool was a separate, stateless request path
# that returned a status JSON rather than the deviation report; replacing it
# with the phase-transition path gives the orchestrator a single, structured
# entry point that enforces the freeze lifecycle.
...
def build_koan_toolset(allowed_names: "frozenset[str] | None" = None) -> Any:
    ...
    Registers koan_set_phase (now with plan_file for the execute handoff),
    koan_suggest_next, koan_set_workflow, the 5 memory tools, the 4 artifact
    tools, the interaction tool (koan_ask_question), and koan_request_scouts.
    koan_request_executor was removed in M4: execution rides on
    koan_set_phase("execute", plan_file=...).
    ...
    async def _koan_set_phase(ctx, phase: str, plan_file: str | None = None) -> str:
        """Transition to a new workflow phase, or tombstone the workflow with 'done'.

        When phase is 'execute', plan_file names the plan artifact to freeze and
        hand off to the executor. The tool blocks until the executor exits and
        returns its deviation report. A bare set_phase('execute') without plan_file
        is rejected.
        """
        return await apply_set_phase(ctx.deps, phase, plan_file)
    ...
    _reg(
        ...
        (
            ...
            "When phase is 'execute', pass plan_file='plan-milestone-N.md' (or 'plan.md') "
            "to freeze the named plan and spawn the executor; the tool blocks until the "
            "executor exits and returns its deviation report."
        ),
    )
    ...
    # koan_request_executor wrapper deleted in M4: execution rides on
    # koan_set_phase("execute", plan_file=...) -- see deletion note above.
